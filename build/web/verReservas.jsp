    ...
    <body>
        ...
        <nav class="navbar navbar-dark bg-dark" aria-label="navbarMenu">
            <div class="container-fluid">
                <a class="navbar-brand">HOTEL · Ver Reservas</a>
                <button
                ...
                <table class="table table-responsive table-bordered" style="border-color: black; border-style: solid;">
                    ...
                    <tbody>
                        ...
                        <tr class="tbBody">
                            ...
                            <%
                                j = 0;
                                while ((id != listaHuespedes.get(j).getReserva().getIdReserva())&&(j < listaHuespedes.size())) {
                                    j++;
                                }
                                if (id == listaHuespedes.get(j).getReserva().getIdReserva()){
                                    dniC = listaHuespedes.get(j).getDniPersona();
                                    idHuesped = listaHuespedes.get(j).getIdPersona();
                                }
                            %>
                            <td><%=dniC%></td>

                            <% String checkIN = listaReservas.get(i).getCheckIn();%>
                            <td><%=checkIN%></td>

                            <% String checkOUT = listaReservas.get(i).getCheckOut();%>
                            <td><%=checkOUT%></td>

                            <%costoTotal = listaReservas.get(i).getPrecioTotal();%>
                            <td><%=costoTotal%></td>

                            <%idHab = listaReservas.get(i).getHabitacion().getIdHabitacion();%>
                            <td><%=idHab%></td>

                            <%idEmp = listaReservas.get(i).getEmpleado().getIdPersona();%>
                            <td><%=idEmp%></td>

                            <td>
                                <form action="Sv_delReserva" method="post" style="display: inline;">
                                    <input type="hidden" name="id" value="<%=id%>"></input>
                                    <input type="hidden" name="idH" value="<%=idHuesped%>"></input>
